...
RUN export LANG=C.UTF-8 && apt-get update && apt-get install -y software-properties-common python-software-properties && add-apt-repository -y ppa:ondrej/apache2 && apt-get update && apt-get -y upgrade && apt-get -y install tzdata && echo 'mysql-server-5.7 mysql-server/root_password password password' | debconf-set-selections && echo 'mysql-server-5.7 mysql-server/root_password_again password password' | debconf-set-selections
RUN apt-get -y install nodejs libcurl4-openssl-dev apache2-dev libapr1-dev libaprutil1-dev libxml2 libxslt-dev build-essential patch libssl-dev mysql-client libmysqlclient-dev apache2 git libreadline-dev
...
RUN a2dissite 000-default && a2ensite rails-default && a2enmod ssl http2
